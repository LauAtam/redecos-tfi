...
describe('BuyGroupsController', () => {
  ...
  describe('getActiveGroups', () => {
    it('should call service with nodeId', async () => {
      ...
      expect(mockBuyGroupsService.getActiveGroups).toHaveBeenCalledWith('node-1');
  ...
  describe('joinGroup', () => {
    it('should call service with user id and dto', async () => {
      ...
      mockBuyGroupsService.joinOrCreateGroup.mockResolvedValueOnce({ id: 'order-1' });

      const mockReq = { user: { id: 'user-1' } };
      const result = await controller.joinGroup(mockReq, mockDto);

      expect(mockBuyGroupsService.joinOrCreateGroup).toHaveBeenCalledWith('user-1', mockDto);
