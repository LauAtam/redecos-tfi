...
describe('BuyGroupsService', () => {
  ...
  describe('getActiveGroups', () => {
      ...
      const mockGroups = [
        {
          ...
          closed_at: null,
          product: { id: 'prod-1', name: 'Yerba 1kg', price: 2000, bulk_size: 10 },
          orders: [{ quantity: 3, status: 'CONFIRMED' }, { quantity: 5, status: 'CONFIRMED' }]
        }
      ];

      mockSupabaseClient.then.mockImplementationOnce((resolve) => resolve({ data: mockGroups, error: null }));
    ...
    it('should throw BadRequestException on database error', async () => {
      mockSupabaseClient.then.mockImplementationOnce((resolve) => resolve({ data: null, error: { message: 'Database error' } }));

      await expect(service.getActiveGroups('node-1')).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('joinOrCreateGroup', () => {
    ...
    it('should throw exception if product does not exist', async () => {
      ...
      await expect(service.joinOrCreateGroup('user-1', mockJoinDto)).rejects.toThrow(
        new BadRequestException('El producto no existe.')
      );
    });

    it('should throw exception if node does not exist', async () => {
      // Product exists
      mockSupabaseClient.single.mockResolvedValueOnce({ data: { price: 2000, bulk_size: 10 }, error: null });
      // Node does not exist
      mockSupabaseClient.single.mockResolvedValueOnce({ data: null, error: { message: 'Not found' } });
  ...
  describe('getMyOrders', () => {
    it('should return user orders successfully', async () => {
      ...
      mockSupabaseClient.order.mockResolvedValueOnce({ data: mockOrders, error: null });
